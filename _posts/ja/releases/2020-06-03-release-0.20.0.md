...
excerpt: Bitcoin Core バージョン 0.20.0が利用可能になりました
date: 2020-06-03
type: releases
layout: release
lang: ja
...
Bitcoin Core バージョン 0.20.0 は以下から入手可能です。
...
#### Bitcoin CoreからBIP61 rejectメッセージが削除される

BIP61を有効にする`-enablebip61`コマンドラインオプションが削除されました。(#17004)

この機能はBitcoin Core 0.18.0からデフォルトでは無効になっています。
...
- テストやBitcoinのP2Pネットワークプロトロルの実装のデバッグでは、Bitcoin Coreの
  最近のバージョンで導入されたログメッセージを検査してください。Bitcoin Coreは
...
- OpenSSLがBitcoin Coreでは使用されなくなりました。(#17265)

- Bitcoin CoreからBIP70のサポートが完全に削除されました。
...
- 新しく`-asmap`設定オプションが追加され、IPアドレスをAutonomous System Numbers (ASN)
  ...
  できるようになりました。詳細については課題 #16599 およびPR #16702 および`bitcoind help`
...
- Bitcoin Coreの起動時に、設定されたすべてのカスタム設定が`debug.log`ファイルに
...
- 「システムログイン時にBitcoin Coreを起動」オプションがmacOSで削除されました。(#17567)
...
- バージョン0.18のリリースノートで[発表された](https://bitcoincore.org/ja/releases/0.18.0/#wallet-gui)
  「既知の問題」が修正されました。この問題は複数のBitcoin CoreウォレットとGUIのコインコントロール機能を
  同時に使用している人に影響がありました。(#18894)

- watch-onlyウォレットについて、送金画面で新しいトランザクションを作成したり、
  トランザクション画面で既存のトランザクションの手数料をバンプすると、
  Partially-Signed Bitcoin Transaction (PSBT)が自動的にシステムクリップボードに
  コピーされます。そしてこれを[HWI](https://github.com/bitcoin-core/HWI)などの
  外部プログラムに貼り付けて署名することができます。Bitcoin Coreの将来のバージョンでは、
...
- Bitcoin Coreの自動生成されたソースコードドキュメントは
  https://doxygen.bitcoincore.org で公開されています。(#17596)
...
- `-getinfo`パラメータを使って実行される`bitcoin-cli`ユーティリティは、
...
- `bitcoin-cli`ユーティリティは、`-stdinwalletpassphrase`パラメータを受け入れるようになりました。
