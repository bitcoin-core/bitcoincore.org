...
permalink: /en/2016/02/26/zkcp-announce/
author: gmaxwell
tags: [privacy]
version: 1
...
- [FIXME](https://www.blocktrail.com/BTC/tx/FIXME)
- [FIXME](https://www.blocktrail.com/BTC/tx/FIXME)
...
- [GGPR'12 paper](http://citeseer.ist.psu.edu/viewdoc/summary;jsessionid=E155270AD5F9DFE9665380E0D0AF0CFA?doi=10.1.1.231.912)
...
<!--
To become a part of the Sudoku buying and selling universe yourself, you'll need this patch to Bitcoin Core to enable wallet support for the atomic swap transactions, and the proof tool-set (written in Rust) from.
...


[real instructions go here; this part is a work in progress]
#as root
apt-get install libprocps-dev libssl-dev #bitcoin core stuff
# now as a normal user
#change to bitcoin core PR when opened
git clone https://github.com/bitcoin/bitcoin hltc-bitcoin
cd hltc-bitcoin
git fetch origin pull/7601/head:pr7601
git checkout pr7601
./autogen.sh ; ./configure --with-incompatible-bdb ; make -j48
#start it
cd ..
curl -sSf https://static.rust-lang.org/rustup.sh | sh -s --channel=nightly --disable-sudo --prefix=~/rust
git clone https://github.com/Electric-Coin-Company/pay-to-sodoku
export PATH=$PATH:~/rust/bin/
cd pay-to-sodoku
./get-libsnark && make
#get 4.vk / 4.pk from gmaxwell
#edit src/main.rs to set the right jsonrpc::client::Client url/passsword
cargo run client 4
