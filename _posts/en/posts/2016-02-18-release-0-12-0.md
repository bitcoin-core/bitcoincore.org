...
9. 185x Faster Block Assembly for Miners
...
## 185x Faster Block Assembly for Miners

Traditionally, block template creation has been quite expensive for miners, requiring high computation times and quite a bit of memory. The high computation time is a consequence of the fact that miners currently have to wait until they receive a block to perform all the consensus critical calculations required to validate the block. The high memory requirements are due to the fact that every transaction in one's memory pool hasd its inputs pulled from what is known as the "coinsviewcache" during block template creation.

With the 0.12 release, consensus critical calculations pertaining to individual transactions are no longer performed all at once when a block as found, but are pre-calculated on all transactions as soon as they hit the memory pool and then cached. This means that as soon as a block is found, most of the calculations have already been performed and the block template can be created extremely quickly. Specifically, this represents a reduction from 2.602 seconds to 0.014 seconds, or 185 times faster than before.
