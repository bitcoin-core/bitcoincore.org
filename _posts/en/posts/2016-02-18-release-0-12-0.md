...
In Bitcoin Core, OpenSSL was traditionally used to validate ECDSA signatures in Bitcoin transactions. Unfortunately, the ECDSA component of OpenSSL has not been that thoroughly tested or extensively reviewed enough for Bitcoin’s standards. Various bugs have been found in OpenSSL over Bitcoin’s lifetime (such as [CVE-2014-3570](https://www.openssl.org/news/secadv/20150108.txt), which was discovered by Greg Maxwell and others), and it became clear over time that we needed a better alternative.

A new ECDSA signature validation library called libsecp256k1 has been developed by members of the Bitcoin Core team and plugged in as a replacement for OpenSSL. It is widely accepted that it is more widely tested and thorougly reviewed than OpenSSL, and also comes with some major speed improvements. 

This library is the result of almost 3 years of complex engineering and it represents a more stable and secure alternative to OpenSSL. Validation is now up to 7x faster on 64-bit architecture, and raw reindexing and block validation now takes less than half the time it did before - a major step forward for validating Bitcoin transactions.
