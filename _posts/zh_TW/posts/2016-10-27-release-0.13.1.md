...
[版本 發布說明]: /zh_TW/releases/0.13.1/
[release notes]: /zh_TW/releases/0.13.1/


我們很高興地[ 發布]比特幣核心 0.13.1的[版本 發布說明]，該版本允許礦工們 發布支援隔離見證（segwit）軟分叉的信號並指明如果軟叉被激活，
哪些節點可以用來驗證segwit交易。
...
- **2015年6月** 發布[Elements Project sidechain][]，其中包括一個可以被稱為“從頭開始”的segwit版本，因為在那時並不知道如何與之前的比特幣軟體兼容，因此也冇打算使其兼容。segwit的這個版本一直被Elements側鏈使用，直到Elements Project切換到使用比特幣核心 0.13.1所提供的版本，因為該版本進行了全麵的測試並且與現有的比特幣軟體兼容。
...
segwit不會對任何在segwit激活時不升級的人（除礦工以外）造成問題。截至比特幣核心 0.13.1版本 發布時，segwit已經在testnet上激活了六個多月，
冇有發現一緻性失誤。
...
此外，在月底，除比特幣核心以外的許多程式 -- 包括btcd全節點和許多常用的挖礦程式 – 都有程式碼準備升級到segwit，並被積極地用於在testnet上生成區塊。
...
從比特幣核心 0.10.0版本開始，節點就被預設隻用來中繼和挖礦啞元素為空值（0x00，也稱為OP_0）的交易。
...
有關在比特幣核心 0.13.1中所做的所有更改的詳細信息，請閱讀[版本 發布說明][release notes]。請訪問[下載頁][download page]或[文件目錄][files directory]下載。


比特幣核心 0.13.1是0.13版本係列中規劃的唯一軟分叉版本。規劃的下一個主要版本是比特幣核心0.14.0，
其功能凍結時間[計畫](https://github.com/bitcoin/bitcoin/issues/8719) 為2017年1月中旬，並在所有測試完成後 發布（通常需要一個多月，
以便留給每個人足夠的測試時間）。
