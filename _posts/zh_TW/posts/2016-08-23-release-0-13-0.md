...
title: 比特幣核心 0.13.0 版本已經PO!
permalink: /zh_TW/2016/08/23/release-0.13.0/
version: 1
excerpt: 我們很高興地宣佈比特幣核心0.13.0版本正式PO了
---
{% include toc.html %}
{% include references.md %}

我們很高興地 [宣佈](https://lists.linuxfoundation.org/pipermail/bitcoin-core-dev/2016-August/000018.html) 比特幣核心0.13.0版本正式PO了. 在此版本6個月的開發周期內，[數十名貢獻者][release notes contributors] 完成了比特幣核心的  [幾百個顯著改進][release notes changelog] . 在本輪版本PO的諸多升級當中，下麵可能是讓礦工、節點營運商和錢包用戶特別感興趣的幾點：
...
比特幣核心0.13.0版本所做更改的全麵列表，請參閱 [版本PO宣告][release notes]. 上文羅列的改進點將在以下章節加以詳細描述. 
...
3.  **與其他功能完全集成 ：** 包含在這個版本中的所有其他功能 – 比如交易費過濾、壓縮區塊傳遞, 下同、父子支付方案，以及為ARM上運行的Linux係統PO的官方二進製可執行文件 – 都與segwit程式碼集成並將有可能在segwit被激活前的兩到幾個月內投入生産，為社區評審和測試發現潛在問題提供了額外的時間 

**更多信息:**

- [版本PO信息][release notes segwit]

- [隔離見證：下一步](https://bitcoincore.org/en/2016/06/24/segwit-next-steps/) BitcoinCore.org 部落格文章

- [BIP141: 隔離見證 (共識層)][BIP141]，關於segwit的技術信息以及 segwit 的激活參數將在哪裏PO的信息. 請參考 BIPs [143][BIP143], [144][BIP144], and [145][BIP145]
...
- [版本PO信息][release notes compact blocks]
...
- [交易費過濾版本PO說明][release notes feefilter]
...
- [版本PO說明][release notes hd wallet]
...
2. 對於用戶來說，礦工更加智能的選擇交易的一個附帶好處是，使接收者激勵礦工挖掘未確認的交易成為可能.
...
這個激勵方案通常被稱為父子支付方案（CPFP）。在最簡單的版本裏，礦工將一個交易及其所有的祖先交易組合在一起，計算其總的每位元組費率，來確定將他們一起挖掘是否能夠支付足夠高的費用，以擊敗礦工希望包括在他下一個區塊裏的其他單個交易.
...
欲了解比特幣核心0.13.0版本的所有變更詳情，請 [閱讀版本PO說明][release notes]. 如欲下載，請訪問 [下載頁麵](https://bitcoincore.org/bin/) 或者 [文件目錄](https://bitcoincore.org/bin/bitcoin-core-0.13.0/).
  
隨著位元核心 0.13.0 版本的PO，我們開始了為期六個月的下一個比特幣核心版本（預計為 0.14.0）的開發。在社區的參與下，我們還會選擇為隔離見證使用BIP9參數並將PO一個完全支援隔離見證的次版本（預計為 0.13.1）.
