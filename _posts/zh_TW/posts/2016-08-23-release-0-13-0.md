...
- **為運行在ARM芯片組上的Linux操作系統開發的正式比特幣核心二進製可執行文件** 讓這些平臺的用戶能夠使用預編譯軟體的優勢，這些軟體都是通過Gitian 確定性編譯和多重證明過程保證的.
...
3.  **與其他功能完全集成 ：** 包含在這個版本中的所有其他功能 – 比如交易費過濾、壓縮區塊傳遞, 下同、父子支付方案，以及為ARM上運行的Linux系統 發布的官方二進製可執行文件 – 都與segwit程式碼集成並將有可能在segwit被激活前的兩到幾個月內投入生産，為社區評審和測試發現潛在問題提供了額外的時間 
...
或者如果您使用的是基於Debian的系統，可以嘗試使用如下指令：

  dpkg-architecture  -q DEB_HOST_GNU_SYSTEM
  
這些二進製可執行文件是為使用 GNU libc6的Linux操作系統準備的；他們無法在安卓或者其他操作系統上預設運行.
